...
# extract column that has certified
...
with open('./input/h1b_input.csv', encoding='utf-8') as f0:
	f = f0.readlines()
...
# counting unique string, saved in dictionary
dict1 = count_name(col1)
dict2 = count_name(col2)

# sorting (desending), also include percentage
dict1_sort = [(k, dict1[k], round(dict1[k]/total_st*100,1) ) for k in sorted(dict1, key=dict1.get, reverse=True)]
dict2_sort = [(k, dict2[k], round(dict2[k]/total_st*100,1) ) for k in sorted(dict2, key=dict2.get, reverse=True)]

# keeping only top 10
dict1_sort = dict1_sort[0:10]
dict2_sort = dict2_sort[0:10]

# sorting alphabetically if percentage is a tie
dict1_sort = sorted( dict1_sort, key=lambda x:(-x[2], x[0])  )
dict2_sort = sorted( dict2_sort, key=lambda x:(-x[2], x[0])  )

with open('./output/top_10_states.txt', 'w') as f:
		f.write('TOP_STATES;NUMBER_CERTIFIED_APPLICATIONS;PERCENTAGE\n')
		for name, val, per in dict1_sort:
			f.write("%s;%i;%2.1f%%\n" % (name, val, per)  )
			
with open('./output/top_10_occupations.txt', 'w') as f:
		f.write('TOP_OCCUPATIONS;NUMBER_CERTIFIED_APPLICATIONS;PERCENTAGE\n')
		for name, val, per in dict2_sort:
			f.write("%s;%i;%2.1f%%\n" % (name, val, per  )  )	
